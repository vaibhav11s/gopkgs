...
func vector2d(x, y float32) Vector2D {
	return Vector2D{x, y}
}
...
func (v *Vector2D) Resize(mag float32) {
	m := v.Mag()
	if m == 0 {
		return
	}
...
func (v *Vector2D) Mult(scalar float32) {
	v.X *= scalar
	v.Y *= scalar
}

// rotate the vector in the direction of the angle
func (v *Vector2D) Rotate(angle float32) {
...
func (v *Vector2D) SetHeading(angle float32) {
	m := v.Mag()
